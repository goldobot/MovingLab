#include "C_CoreSlam_8bppMap.h"

C_CoreSlam_8bppMap::C_CoreSlam_8bppMap(uint32_t globalMapSquareSize__mm, uint32_t lidarMinReliableMeasuredDist__mm, uint32_t lidarMaxReliableMeasuredDist__mm, uint32_t lidarMinDistToAddVirtualMeasures__mm, uint32_t maxNbOfVirtualMeasuresAtMaxDist)
{
    // Store the map size
...
C_CoreSlam_8bppMap::~C_CoreSlam_8bppMap()
{
...
void C_CoreSlam_8bppMap::Reset()
{
    ...
    m_ptThnlGlobalMap8bpp_0 = C_Tools_DataTable::AllocArrayU8(m_opti_thnlGlobalMapSquareSize__pixel, m_opti_thnlGlobalMapSquareSize__pixel,  0);
    m_ptThnlGlobalMap8bpp_1 = C_Tools_DataTable::AllocArrayU8(m_opti_thnlGlobalMapSquareSize__pixel, m_opti_thnlGlobalMapSquareSize__pixel,  0);
    ...
    m_currentPosX__pixel = C_Tools_Maths::C_Tools_Maths::Opti_round(m_currentPosX__mm * CONV_MM_2_PIXEL);
    m_currentThnlPosX__pixel = C_Tools_Maths::Opti_round(m_currentPosX__mm * THNL_CONV_MM_2_PIXEL);
    ...
    m_currentPosY__pixel = C_Tools_Maths::Opti_round(m_currentPosY__mm * CONV_MM_2_PIXEL);
    m_currentThnlPosY__pixel = C_Tools_Maths::Opti_round(m_currentPosY__mm * THNL_CONV_MM_2_PIXEL);
    ...
    double deltaAngleInRad = MATHS_2PI / ((double)(LIDAR_NB_OF_MEASURES_PER_ROTATION));
...
void C_CoreSlam_8bppMap::IntegrateNewSensorData(uint32_t * tabOfDistMeasures__mm, int offsetX__mm, int offsetY__mm, enum_EstimationType estimationType)
{
...
bool C_CoreSlam_8bppMap::EstimateNewPosition(enum_EstimationType estimationType)
{
    ...
    m_currentPosX__pixel = C_Tools_Maths::Opti_round(m_currentPosX__mm * CONV_MM_2_PIXEL);
    m_currentThnlPosX__pixel = C_Tools_Maths::Opti_round(m_currentPosX__mm * THNL_CONV_MM_2_PIXEL);
...
bool C_CoreSlam_8bppMap::ProcessLidarData(int offsetX__mm, int offsetY__mm)
{
...
uint64_t C_CoreSlam_8bppMap::ComputeMatching(double posX__mm, double posY__mm, double preComputedCos, double preComputedSin)
{
...
uint64_t C_CoreSlam_8bppMap::Opti_ComputeMatching(double xInMm, double yInMm)
{
#ifdef ENABLE_MAP_BOUNDARIES_CHECK
...
void C_CoreSlam_8bppMap::UpdateMap()
{
    double x2p, y2p;
    int32_t x2, y2, xp, yp;
    double add;
...
void C_CoreSlam_8bppMap::UpdateThumbnailMap()
{
    double x2p, y2p;
    double x2p_0, y2p_0;
    double x2p_1, y2p_1;
    ...
    while (i >= 0)
    {
        ...
        x2 = C_Tools_Maths::Opti_floor(currentX__pixel + x2p_0 + 0.5);
        y2 = C_Tools_Maths::Opti_floor(currentY__pixel + y2p_0 + 0.5);
        ...
        x2 = C_Tools_Maths::Opti_floor(currentX__pixel + x2p_1 + 0.5);
        y2 = C_Tools_Maths::Opti_floor(currentY__pixel + y2p_1 + 0.5);
...
void C_CoreSlam_8bppMap::RayTraceLidarData(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t xp, int32_t yp)
{
...
void C_CoreSlam_8bppMap::ThumbnailRayTraceLidarData(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t xp, int32_t yp)
{
    int32_t x2c, y2c, dx, dy, dxc, dyc, error, errorv, derrorv, x;
...
void C_CoreSlam_8bppMap::ThumbnailRayTraceInstantLidarData(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t xp, int32_t yp)
{
    int32_t x2c, y2c, dx, dy, dxc, dyc, error, errorv, derrorv, x;
