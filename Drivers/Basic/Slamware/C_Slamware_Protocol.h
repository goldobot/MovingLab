#ifndef C_SLAMWARE_PROTOCOL_H
#define C_SLAMWARE_PROTOCOL_H
...
class C_Slamware_Protocol: public QObject
{
    Q_OBJECT

public:
    // Constructor
    C_Slamware_Protocol(void)
    {
        // Send timeout management
        //connect(&m_manageConnectionTimer, &QTimer::timeout, this, &C_Slamware_Protocol::SLOT_ManageConnection);
    }

    // Set the COM port
    void SetComPort(C_Socket * ptComPort)
    {
        m_ptComPort = ptComPort;
        connect(m_ptComPort, &C_Socket::SIG_NewDataReceived, this, &C_Slamware_Protocol::SLOT_ProcessReceivedData);

        m_ptComPort->doConnect();
    }
    ...
    bool AddStartScanningSequence(void)
    {
        ...
        AddCommand("LR03", C_Slamware_Protocol::HIGH_PRIORITY_MANUAL_CMD);

        // Init the device scan speed (5 rps)
        AddCommand("MS05", C_Slamware_Protocol::HIGH_PRIORITY_MANUAL_CMD, 8000);

        // Start scanning
        AddCommand("DS", C_Slamware_Protocol::NORMAL_PRIORITY_MANUAL_CMD);
