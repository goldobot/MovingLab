...
C_ScanseLidar::C_ScanseLidar(QWidget * ptParent) : QMainWindow(ptParent)
{
    // Configure the HMI
    m_userInterface.setupUi(this);
    m_comPort.SetWindowTitle("Scanse Lidar COM port");
    ...
    connect(&m_comPort, &C_SerialCom::SIG_ClosingWindow, this, &C_ScanseLidar::SLOT_ClosingComPortWindow);

    // Date flow connection
    connect(&m_lidarProtocol, &C_ScanseLidar_Protocol::SIG_NewLidarData, this, &C_ScanseLidar::SLOT_StoreNewScanData);

    // Activate modules monitoring
    connect(&m_monitoringTimer, &QTimer::timeout, this, &C_ScanseLidar::SLOT_PeriodicCheckModulesMalFunction);
    m_monitoringTimer.start(3000);

    connect(&m_TIMER_processNewScanData, &QTimer::timeout, this, &C_ScanseLidar::SLOT_ProcessNewScanData);
}


void C_ScanseLidar::SLOT_StoreNewScanData(int sensorId, uint32_t * tabOfMeasures__mm, double * tabOfMeasures__cos, double * tabOfMeasures__sin, uint32_t nbOfMeasures)
{
    Q_UNUSED(sensorId)
...
void C_ScanseLidar::SLOT_ProcessNewScanData(void)
{
...
void C_ScanseLidar::DrawLidarScans(void)
{
    ...
    {
        x = static_cast <int>((* ptAngle__cos) * (* ptDist) * CONV_MM_2_PIXEL) + screenCenterX + SCANSE_LIDAR_OFFSET_X__pixel;
        y = static_cast <int>((* ptAngle__sin) * (* ptDist) * CONV_MM_2_PIXEL) + screenCenterY + SCANSE_LIDAR_OFFSET_Y__pixel;
...
void C_ScanseLidar::on_btn_ShowComPort_clicked()
{
...
void C_ScanseLidar::on_btn_Connect_clicked()
{
...
void C_ScanseLidar::on_btn_MoveDownLdsScan_clicked()
{
    m_viewCenterY -= 10.0;
    m_userInterface.gv_LdsScan->centerOn(m_viewCenterX, m_viewCenterY);
}

void C_ScanseLidar::on_btn_MoveUpLdsScan_clicked()
{
    m_viewCenterY += 10.0;
    m_userInterface.gv_LdsScan->centerOn(m_viewCenterX, m_viewCenterY);
}

void C_ScanseLidar::on_btn_MoveLeftLdsScan_clicked()
{
    m_viewCenterX += 10.0;
    m_userInterface.gv_LdsScan->centerOn(m_viewCenterX, m_viewCenterY);
}

void C_ScanseLidar::on_btn_MoveRightLdsScan_clicked()
{
    m_viewCenterX -= 10.0;
    m_userInterface.gv_LdsScan->centerOn(m_viewCenterX, m_viewCenterY);
}

void C_ScanseLidar::on_btn_CenterLdsScan_clicked()
{
...
void C_ScanseLidar::on_btn_ZoomIn_clicked()
{
    m_userInterface.gv_LdsScan->scale(1.1, 1.1);
}

void C_ScanseLidar::on_btn_ZoomOut_clicked()
{
    m_userInterface.gv_LdsScan->scale(0.9, 0.9);
}

void C_ScanseLidar::on_btn_ZoomDefault_clicked()
{
...
void C_ScanseLidar::closeEvent (QCloseEvent * ptEvent)
{
    Q_UNUSED(ptEvent);
...
void C_ScanseLidar::SLOT_ClosingComPortWindow(void)
{
    m_userInterface.btn_ShowComPort->setText("Show COM port");
}

void C_ScanseLidar::SLOT_PeriodicCheckModulesMalFunction()
{
